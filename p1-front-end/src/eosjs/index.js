...
export const requireCharge = async (quantity, payment_code, payer) => {
    ...
    console.log(chargeContract);
    console.log(`requireCharge:`,charger, `${quantity} ${Token_Symbol}`, payment_code, payer);
    const trans = await chargeContract.reqcharge(charger, `${quantity} ${Token_Symbol}`, `${payment_code}`, payer, {authorization: charger});
    console.log(trans);
    return !!trans;
};
